...
#define OPENFILE    "out.txt"   //зашифрованный файл, который будет расшифровываться
#define TMPFILE     "tmp.txt"   //временный файл с добавочными байтами
#define CRYPTEDFILE "out2.txt"  //расшифрованный файл
...
{    
    ...
    in.seekg(0, ios::end);       
    long int size1 = in.tellg();    //размер дополненного файла в байтах
    in.seekg(0, ios::beg);
	in.close();
	
	ifstream key (KEYFILE, ios::in | ios::binary);    //считывание ключа
	for(int i = 0; i < 4; i++)
    {
    	for(int j = 0; j < 8; j++)
    	{
    ...
    ifstream cryptedfile (OPENFILE, ios::in | ios::binary);
    ofstream tmpfile (TMPFILE, ios_base::app | ios::binary);
    for(int k = 0; k < (size1/16); k++)     //основоной цикл штфрования
    {
    	encrypt();
    	cryptedfile.read((char*)OPENBLOCK,sizeof(OPENBLOCK));
    	for(int i = 0; i < 16; i++)
    	{
    		OUTRES[i] = (OUT[i] ^ OPENBLOCK[i]);
    		tmpfile.put((char)OUTRES[i]);
    		LR[i] = OPENBLOCK[i];
		}
	}
	tmpfile.close();
	...
	ifstream size (SIZEFILE, ios::in);  //размер исходного файла
    unsigned long int true_size;
    size >> true_size;
    size.close();
	
	ifstream tmpfile2 (TMPFILE, ios::in | ios::binary);
	ofstream out2 (CRYPTEDFILE, ios_base::app | ios::binary);
	for(int i = 0; i < true_size; i++)
    {
    	char buffer;
		tmpfile2.read(&buffer, sizeof(char));
		out2.put((char)buffer);
	}
	tmpfile2.close();
	out2.close();
	remove(TMPFILE);
